...
export default class extends AbstractMigration<ClientPostgreSQL> {
  ...
  async up(_info: Info): Promise<void> {
    await this.client.queryArray(`
        ...
        -- table for queue backup, import jobs
        -- frontend call API to check job status and notify when done
        CREATE TABLE background_jobs (
            job_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            user_id UUID REFERENCES users(user_id) ON DELETE CASCADE,
            type VARCHAR(50) NOT NULL, -- 'BACKUP_PDF', 'IMPORT_JSON'
            status VARCHAR(20) DEFAULT 'PENDING', -- 'PENDING', 'PROCESSING', 'COMPLETED', 'FAILED'
            result_url TEXT, -- Link file download sau khi backup xong
            error_message TEXT,
            created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
        );
